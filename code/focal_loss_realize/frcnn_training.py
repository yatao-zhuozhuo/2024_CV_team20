...
def focal_loss(score,label,weight=None,gamma=0,ignore_index=-100):
    #print(score.shape,label.shape)
    if weight!=None:
        weight=weight.to(device)
    ce_loss=F.cross_entropy(score,label,weight=weight,ignore_index=ignore_index,reduction='none')
    ...
    #print('p: ',p)
    rows=torch.arange(p.shape[0])
    p=p[rows,label]
    #print('select p: ',p)
    loss=torch.pow(1-p,gamma)*ce_loss
    return loss.mean()
    
    #label=F.one_hot(label,num_classes=score.shape[1])
    #print(p,'\n',label,'\n',torch.log(p),'\n',torch.pow(1-p,gamma))
    #loss=-alpha*torch.pow(1-p,gamma)*torch.log(p)*label
...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        for i in range(n):
            ...
            #weight=torch.tensor([2,1,2,1,1,1],dtype=torch.float32)
            #rpn_cls_loss = focal_loss(rpn_score, gt_rpn_label, weight=weight, gamma=2, ignore_index=-1)
        ...
        for i in range(n):
            ...
            weight=torch.tensor([2,1,2,1,1,1],dtype=torch.float32)
            roi_cls_loss = focal_loss(roi_score, gt_roi_label,weight=None,gamma=2)
